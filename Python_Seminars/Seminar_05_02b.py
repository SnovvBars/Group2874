...
def print_greet(player, min, max, ostatok):                                 # Поочередный ход с проверкой количества
    ...
    elif player == 2: count_taken = num_bot_random(min, max)                                #  Если бот
...
def num_bot_random(min, max):                                      # Добавлена функция генератора чисел для бота
    candy = randint(min, max + 1)
...
player_1 = input("\nИмя игрока: ")
player_2 = "Бот"                                    # БОТ ВОРОЙ (ПО НОМЕРУ) ИГРОК
...
current_player = randint(1, 2)              # Случайный выбор игрока
